...
10  format('    <stat type="',A,'" by="nodesum">',F0.9,'</stat>')
  ...
  subroutine color_by_hash(name,namelen,commin,comm_name,comm_hosts,rank_world,ierr)
    ...
    hashtries: do itry=1,len(ctry)
       ...
       hash=c_crc32c
! 
       ! xyc added to convert the signed Fortran int. to unsigned C int.:
       if (hash < 0) then
          hash = -hash
       elseif (hash > 0) then
          hash = 1+ hash
...
!    XYC Debugging:
      ! write(*,*) 'entering MPI_COMM_SPLIT'
      ! write(*,*) commin, hash, rank_world, comm_name
!   problem identified: hash is negative value, which is wrong. 
