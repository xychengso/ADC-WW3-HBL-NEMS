...
!/
      ...
      CHARACTER(LEN=512), PARAMETER  :: SWITCHES  = &
                    'F90 NOGRB NC4 DIST MPI PDLIB SCRIP PR3 UQ FLX0 SEED ST4 FLD1 STAB0 NL1 BT4 DB1 MLIM TR0 BS0 XX0 WNX1' // &
...
!
              IF ( FLGRDALL( 5, 1) ) THEN
                  IH     = IH + 1
                  IT     = IT + 1
      CALL MPI_SEND_INIT (USTFLD(1),NSEALM , MPI_REAL, IROOT,    &
                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
                  IH     = IH + 1
                  IT     = IT + 1
      CALL MPI_SEND_INIT (USTDIRFLD(1),NSEALM , MPI_REAL, IROOT,    &
                                IT, MPI_COMM_WAVE, IRQGO(IH), IERR)
                  IH     = IH + 1
                  IT     = IT + 1
...
!
                IF ( FLGRDALL( 5, 1) ) THEN
                    IH     = IH + 1
                    IT     = IT + 1
      CALL MPI_RECV_INIT (USTFLD   (I0),1,WW3_FIELD_VEC, IFROM, IT,  &
                               MPI_COMM_WAVE, IRQGO2(IH), IERR )
                    IH     = IH + 1
                    IT     = IT + 1
      CALL MPI_RECV_INIT (USTDIRFLD(I0), 1, WW3_FIELD_VEC, IFROM,   &
