...
!         write(*,*) "debug: Call into FricVel"
        call CalcFricVel_v2( expField(i1),expField(i2), rc )
        if (ESMF_LogFoundError(rc, PASSTHRU)) return
      endif
!
! -------------------------------------------------------------------- /
...
!     layout, whereas for unstructured meshes it is a 1D array
      ...
      if ( LPDLIB == .FALSE. ) then
        ...
        jsea_loop2: do jsea = 1,np
          ...
          sxyn(jsea) = sxy(isea)
          syyn(jsea) = syy(isea)
        write(msg,'(A15, I6)') trim(cname)//' isea', isea
        call ESMF_LogWrite(trim(msg), ESMF_LOGMSG_INFO)
        enddo jsea_loop2
      endif
...
!      subroutine CalcFricVel (a, ustxField, ustyField, rc )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |        T. J. Campbell, NRL        |
...
!#ifdef USE_W3OUTG_FOR_EXPORT
!         for ssd wind stress:
          ustxn(jsea)= UST(isea)*cos(USTDIR(isea))
          ustyn(jsea)= UST(isea)*sin(USTDIR(isea))
!           ustxn(jsea)=1.0
!           ustyn(jsea)=1.0
          ...
          !   call wnd2ustadcbulk(U10(isea), U10D(isea),ustxn(jsea), ustyn(jsea))
          !endif
!#else
...
!#endif
        ...
        jsea_loop2: do jsea = 1,np
          isea = iplg(jsea)
! ------     To Figure out: SPEC? CG1? FPI?, VSIN?
! ------     U10ABS, HSTOT
! *****  SPEC: 1D wave action spectrum --> so need to turn a(nth,nk,*)
! *****        to 1D version;
! *****  CG1: group velocity CG(1:NK,ISEA)
! *****  FPI: output, needs to be defined as the variable;
! *****  VSIN: 1D source term  (this will need to be computed..) -->
! this is not a good way to go. Ideally, I should have the sea state
! dependent wind stress computed already for use by the ww3 cap.
 
          !use qcflag to further perform quality control of the
          !frictional velocity;
          ! use bulk frictional velocity for ADCIRC
             call wnd2ustadcbulk(U10(jsea), U10D(jsea),ustxn(jsea), ustyn(jsea))
        enddo jsea_loop2
      endif
...
!/ End of CalcFricVel ----------------------------------------------- /
#undef METHOD
#define METHOD "CalcFricVel_v2"
      subroutine CalcFricVel_v2 (ustxField, ustyField, rc )
!      subroutine CalcFricVel (a, ustxField, ustyField, rc )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
...
!#endif
        ...
        jsea_loop2: do jsea = 1,np
          isea = iplg(jsea)
          ustxn(jsea)= UST(jsea)*cos(USTDIR(jsea))
          ustyn(jsea)= UST(jsea)*sin(USTDIR(jsea))
