...
!/
     ...
     REAL, INTENT(OUT)                 :: FLD_USTAR, FLD_USTDIR
      INTEGER, INTENT(OUT)                 :: FLD_QC
!/
!/ ------------------------------------------------------------------- /
!/ Local parameters
!/
...
!
      IF (U10ABS.GT.10. .and. HSTOT.gt.0.5) then
          CALL W3FLD1 ( SPEC,min(FPI/TPI,2.0),COEF*U10ABS*COS(U10DIR)/WIND_FAC,        &
                    COEF*U10ABS*Sin(U10DIR)/WIND_FAC, ZWND, DEPTH, 0.0, &
                    FLD_USTAR, FLD_USTDIR, Z0,TAUNUX,TAUNUY,CHARN, FLD_QC)
          FLD_USTAR=FLD_USTAR/COEF
      ELSE
          CHARN = AALPHA
          FLD_QC=999
      ENDIF
