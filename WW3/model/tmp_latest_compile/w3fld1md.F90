...
!       FLAG    Inte   O  quality control flag (0=non-converged FLD
!                           iteration)
!     ----------------------------------------------------------------
!
!  4. Subroutines used :
...
!XYC added:
      real::   TARGETLEV, TAILEVMAX
      real::   BK(NK), EK(NK)
      real::   MSSX, MSSY, MSS, MSSX_elm, MSSY_elm
      real::  SIGfine(65), addlocflag(3), transID(3),sig_tmp(63)
      real:: freq_a, freq_b, freq_c, tmp1, tmp2,spcintp
      integer:: extpnt, NKT0, KMAX0,I2 ,cnt
!/
!/ ------------------------------------------------------------------- /
!/
!
...
! XC modified, so we can switch between old GFDL drag and HWRF drag
       else
          CALL WND2SAT(U10,SAT,Tail_Choice)
       endif
!
! 1.  Attach Tail ---------------------------------------------------- *
!
      ...
      freq_a=FPI*TPI*tail_transition_ratio1
      freq_b=FPI*TPI*tail_transition_ratio2
      freq_c=sqrt(2.0)*freq_b
 
!      write(*,*) freq_a, freq_b, freq_c, SIG(NK)
 
!    build extended sig_temp
 
      sig_tmp(1:nk)=sig(1:nk)
      call sig2wn(sig(nk),depth,kmax)
      NKT0=NK
      DO WHILE (Kmax .LT. 366.0)
         NKT0=NKT0+1
         sig_tmp(NKT0)=sig(nk)* XFR**(NKT0-NK)
         kmax=kmax* XFR**2
      ENDDO
 
!      write(*,*) NKT0
 
! i. determine the extact wavenumber of the KtailA, KtailB, and KtailC:
     !** i. Find freq for beginning spc level transition to tail
      call sig2wn (FPI*TPI*tail_transition_ratio1,depth,ktaila )
     ! then find the neighboring wavenumbers for KtailA:
      KA1 = 2     ! Do not modify 1st wavenumber bin
      DO WHILE ( (  freq_a .GE. SIG_tmp(KA1) ) .AND. (KA1 .LT. NKT0-6) )
        KA1 = KA1 + 1
      ENDDO
      ...
      IF ( (freq_a-SIG_tmp(KA1-1) .GT. 0.01*(SIG_tmp(KA1)-SIG_tmp(KA1-1)) )  &
         .AND. (SIG_tmp(KA1)-freq_a .GT.0.01*(SIG_tmp(KA1)-SIG_tmp(KA1-1)) )) &
         THEN
         extpnt=extpnt+1
         SIGfine(KA1-1+extpnt)=freq_a
         addlocflag(1)=1
        ! record the indx of freq_a
         transID(1)=KA1
      else if ( freq_a-SIG_tmp(KA1-1) .LE.0.01*(SIG_tmp(KA1)-SIG_tmp(KA1-1)))&
            then
         transID(1)=KA1-1
      else
         transID(1)=KA1
     ...
     ! then find the neighboring wavenumbers for KtailB:
        KA2 = KA1+2
        DO WHILE ( ( freq_b .GE. SIG_tmp(KA2) ) .AND. (KA2 .LT. NKT0-4) )
          KA2 = KA2 + 1
        ENDDO
 
      DO K=KA1,KA2-1
         SIGfine(K+extpnt)=SIG_tmp(K)
      ENDDO
 
      IF ( ((SIG_tmp(KA2)-freq_b) .GT. 0.01*(SIG_tmp(KA2)-SIG_tmp(KA2-1)))  &
         .AND. ((freq_b-SIG_tmp(KA2-1)).GT.0.01*(SIG_tmp(KA2)-SIG_tmp(KA2-1)))) &
         THEN
         extpnt=extpnt+1
         SIGfine(KA2-1+extpnt)=freq_b
         addlocflag(2)=1
         transID(2)=KA2-1+extpnt
      ELSE IF ((freq_b-SIG_tmp(KA2-1)).LE.0.01*(SIG_tmp(KA2)-SIG_tmp(KA2-1))) &
              THEN
         transID(2)=KA2-1+extpnt
      ELSE
         transID(2)=KA2+extpnt
      ENDIF
 
 
     !iii. Find wavenumber for ending spc direction transition to tail
         KA3=KA2+2
         DO WHILE ( ( freq_c .GE. SIG_tmp(KA3) ) .AND. (KA3 .LT. NKT0-2) )
            KA3 = KA3 + 1
         ENDDO
         ...
         IF ( ((SIG_tmp(KA3)-freq_c) .GT. 0.01*(SIG_tmp(KA3)-SIG_tmp(KA3-1)))  &
           .AND. ((freq_c-SIG_tmp(KA3-1)).GT.0.01*(SIG_tmp(KA3)-SIG_tmp(KA3-1)))) &
         THEN
           extpnt=extpnt+1
           SIGfine(KA3-1+extpnt)=freq_c
           addlocflag(3)=1
           transID(3)=KA3-1+extpnt
         ELSE IF ((freq_c-SIG_tmp(KA3-1)).LE.0.01*(SIG_tmp(KA3)-SIG_tmp(KA3-1))) &
              THEN
           transID(3)=KA3-1+extpnt
!          addlocflag(3)=0
         ELSE
           transID(3)=KA3+extpnt
!          addlocflag(3)=0
         ENDIF
...
!
      ...
      DO K = 1, NKT !existing spectrum
         call sig2wn(sigfine(k),depth,wn(k))
         CP(K) = ( SIGfine(K) / WN(K) )
         sig2(k) = sigfine(k)
      ENDDO!K
...
!        ENDDO!T
          ! I is the index to get the corresponding discrete action
          ! density spectra:
        if (((cnt+1==transID(1)-1).and. (addlocflag(1)==1))    &
           .or. ((cnt+1==transID(2)-1).and.(addlocflag(2)==1)) &
           .or. ((cnt+1==transID(3)-1).and.(addlocflag(3)==1))) then
           ...
           DO T=1, NTH
              I = T+(K-1)*NTH
              ! compute the energy spectrum at this wavenumber
              tmp1=ASPC(I) * SIG(K)
              I2=T+K*NTH
              tmp2=ASPC(I2) * SIG(K+1)
              call linear_interp(WN(cnt), WN(cnt+2), tmp1,tmp2,WN(cnt+1),spcintp)
              SPC2(cnt,T)=tmp1
              SPC2(cnt+1,T)=spcintp
           ENDDO
           ! this one is adding the inserted k:
!                write(*,'(A12,1x, I2, 1x, A3, f6.2)') 'debug:SPC2(', cnt, ')=',sum(SPC2(cnt,1:NTH))
!                write(*,'(A12,1x, I2, 1x, A3, f6.2)') 'debug:SPC2(', cnt+1, ')=',sum(SPC2(cnt+1,1:NTH))
           cnt=cnt+1
 
 
 
        else ! when WN(K) is the discrete wavenumber based on sig
 
           cnt=cnt+1
           DO T=1, NTH
              I = T+(K-1)*NTH
              SPC2(cnt,T) = ASPC(I)  * SIG(K)
           ENDDO!T
...
!          SPC2(K,T)=SPC2(NK,T)*WN(NK)**3.0/WN(K)**(3.0)
           SPC2(K,T)=SPC2(cnt,T)*( WN(cnt)**3.0/WN(K)**3.0)
        ENDDO!T
      ENDDO!K
!
          ...
          DO ZI = 2, NKT
            ...
            DO T = 1, NTH
              ...
              TLTND(ZI) =TLTND(ZI)+( SIN( TH(T) ) * COS( ANGDIF )**2)&
                          ...
                          * ( WN(Z2)**2.0 )*DTH /tanh(WN(Z2)*DEPTH)
           ...
           Z2=NKT+1
            DO ZI=1, NKT
              ...
              DO T=1, NTH
                ...
                ! I need to prevent CP(Z2)==0; How could I deal with
                ! this?
                IF ( CP(Z2) .LE. 10**(-4) ) THEN
                     write(*,*) 'CP(Z2)=', CP(Z2)
                     BETAG=0.0
                ELSE
...
!|---------------------------------------------------------------------|
      ...
      IF (PRESENT(CHARN)) THEN
         CHARN = 0.01/SQRT(SQRT( TAUNUX**2 + TAUNUY**2 )/DAIR/(UST**2))
      ENDIF
...
!/ ------------------------------------------------------------------- /
      SUBROUTINE WND2SAT(WND10,SAT,Tail_Choice)
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |           B. G. Reichl            |
!/                  |                        FORTRAN 90 |
...
!     ----------------------------------------------------------------
      ...
      !Input: Tail_Choice - control which tail functions to use.
...
!/
        ...
        INTEGER, INTENT(IN) :: Tail_Choice
        REAL, INTENT(OUT) :: SAT
!
       IF (Tail_Choice==1) THEN
...
!        ENDIF
        IF (WND10<20.0) THEN
           SAT = 0.0000000131913461 * WND10**2 + &
                 0.000229956367 * WND10     + &
                 0.00550800211
        ELSEIF (WND10<50.0) THEN
           SAT = 0.00000000559101083 * WND10**4   &
                 -0.000000549949516 * WND10**3  &
                 +0.0000130093399 * WND10**2  &
                 -0.0000953766876 * WND10     &
                 +0.0102647903
        ELSE
           SAT = -0.0000000354155805 * WND10**3  &
                 +0.0000136036043 * WND10**2  &
                 -0.00129398317 * WND10     &
                 +0.0393321920
        ENDIF
      ...
      ELSEIF (Tail_Choice .eq. 3) Then
      ! ADCIRC Garratt 1977 capped at 0.0028
         IF (WND10<=19.0 ) THEN
             SAT= -0.00000521781563 * WND10**2 &
                  +0.000317144478 * WND10  &
                  +0.00566955045
         ELSE
             SAT= 0.00000000218597644 * WND10**4 &
                  -0.000000412577288 * WND10**3 &
                  +0.0000305055798 * WND10**2 &
                  -0.00110615766 * WND10 &
                  +0.0222230368
         ENDIF
      ENDIF
...
!
      SUBROUTINE linear_interp(x1,x2,y1,y2, xq,yq)
      IMPLICIT NONE
       real, intent(in):: x1,x2, y1,y2, xq
       real, intent(out):: yq
 
        yq= (x2-xq)/(x2-x1)*y1 + (xq-x1)/(x2-x1)*y2
