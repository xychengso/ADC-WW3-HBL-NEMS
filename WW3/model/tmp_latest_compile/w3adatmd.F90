...
!
       ...
       INTEGER, POINTER       :: QCFLG(:)
       INTEGER, POINTER       :: XQCFLG(:)
!
...
!
      ALLOCATE ( WADATS(IMOD)%CHARN   (NSEALM),                       &
                 WADATS(IMOD)%USTFLD  (NSEALM),                       &
                 WADATS(IMOD)%USTDIRFLD (NSEALM),                     &
                 WADATS(IMOD)%QCFLG   (NSEALM),                       &
      ...
      CHECK_ALLOC_STATUS ( ISTAT )
!
      WADATS(IMOD)%USTFLD   = UNDEF
      WADATS(IMOD)%USTDIRFLD  = UNDEF
      WADATS(IMOD)%QCFLG    = 999
...
!
      IF ( OUTFLAGS( 5, 1) ) THEN
          ALLOCATE ( WADATS(IMOD)%XUSTFLD(NXXX), STAT=ISTAT )
          CHECK_ALLOC_STATUS( ISTAT )
          ALLOCATE ( WADATS(IMOD)%XUSTDIRFLD(NXXX), STAT=ISTAT )
          CHECK_ALLOC_STATUS( ISTAT )
          ALLOCATE ( WADATS(IMOD)%XQCFLG(NXXX), STAT=ISTAT )
          CHECK_ALLOC_STATUS( ISTAT )
        ELSE
          ALLOCATE ( WADATS(IMOD)%XUSTFLD(1), STAT=ISTAT )
          CHECK_ALLOC_STATUS( ISTAT )
          ALLOCATE ( WADATS(IMOD)%XUSTDIRFLD(1), STAT=ISTAT )
          CHECK_ALLOC_STATUS( ISTAT )
          ALLOCATE ( WADATS(IMOD)%XQCFLG(1), STAT=ISTAT )
          CHECK_ALLOC_STATUS( ISTAT )
        END IF
...
!
      IF ( OUTFLAGS( 5, 11) ) THEN
        ...
        END IF
!
      WADATS(IMOD)%XUSTFLD = UNDEF
      WADATS(IMOD)%XUSTDIRFLD = UNDEF
      WADATS(IMOD)%XQCFLG  = 999
...
!
          ...
          PEP    => WADATS(IMOD)%PEP
!
          USTFLD   => WADATS(IMOD)%USTFLD
          USTDIRFLD   => WADATS(IMOD)%USTDIRFLD
          QCFLG    => WADATS(IMOD)%QCFLG
...
!
          USTFLD   => WADATS(IMOD)%XUSTFLD
          USTDIRFLD=> WADATS(IMOD)%XUSTDIRFLD
          QCFLG    => WADATS(IMOD)%XQCFLG
