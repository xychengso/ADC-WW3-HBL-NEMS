...
module yowNodepool
  ...
  type(llist_type), public, allocatable :: ARR_ipgl(:)
  ...
  type, public :: t_Node
    ...
    !> X Coordiante
    real(rkind), pointer :: x=>null()
    !> Y Coordiante
    real(rkind), pointer :: y=>null()
    !> Z Coordiante
    real(rkind), pointer :: z=>null()
  ...
  !> coordinates of the node, ({x,y,z}, global ID)
  !> \todo change to local ID, later
  real(rkind), public, target, allocatable :: xyz(:,:)
