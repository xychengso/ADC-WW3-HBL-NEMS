...
!          IF (JSEA == 10000) WRITE(*,'(2I20,10F20.10)') JSEA, ISEA, SUM(VSTOT(:,JSEA)), SUM(VDTOT(:,JSEA)), SUM(VSDB),SUM(VDDB), DE
          DO IK=1,NK
            DO ITH=1,NTH
              ISP=ITH + (IK-1)*NTH
...
!!/DEBUGFREQSHIFT        END DO
            ...
            IF (IMEM == 2) THEN
              ...
              !  WRITE(*,'(8I10,4F20.10)') IP, JSEA, ISEA, NSEA, NSEAL, np, npa, IP_glob, SUM(ASPAR_DIAG), SUM(ASPAR_DIAG_LOCAL), SU
              !  CALL PDLIB_ABORT(25)
              !ENDIF
