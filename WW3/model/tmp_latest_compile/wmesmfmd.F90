...
!#define USE_W3OUTG_FOR_EXPORT___enabled
!/
!/ ------------------------------------------------------------------- /
...
#define TEST_WMESMFMD_FricVel___disabled
...
! XYC added sea-state dependent wind stress:
      i = i + 1
      if ( istep.eq.2 ) then
        expFieldName(i)    = 'uustss'
        expFieldStdName(i) = 'eastward_sea_state_dependent_fric_vel'
        expFieldDim(i)     = 2
      endif
 
      i = i + 1
      if ( istep.eq.2 ) then
        expFieldName(i)    = 'vustss'
        expFieldStdName(i) = 'northward_sea_state_dependent_fric_vel'
        expFieldDim(i)     = 2
      endif
...
! XYC added sea-state dependent wind stress:
      i1 = FieldIndex( expFieldName, 'uustss', rc )
      if (ESMF_LogFoundError(rc, PASSTHRU)) return
      i2 = FieldIndex( expFieldName, 'vustss', rc )
      if (ESMF_LogFoundError(rc, PASSTHRU)) return
      if ( expFieldActive(i1) .and. &
           expFieldActive(i2) ) then
        write(msg,'(a,a10,a10)') trim(cname)//': ', &
          trim(expFieldName(i1)),trim(expFieldName(i2))
        call ESMF_LogWrite(trim(msg), ESMF_LOGMSG_INFO)
        flogr2(5,1) = .true.  !frictional velocity UST
      endif
 
! XYC added ssd roughness length (to ATM):
      i1 = FieldIndex( expFieldName, 'z0rlen', rc )
      if (ESMF_LogFoundError(rc, PASSTHRU)) return
      if ( expFieldActive(i1) ) then
        write(msg,'(a,a10)') trim(cname)//': ', &
          trim(expFieldName(i1))
        call ESMF_LogWrite(trim(msg), ESMF_LOGMSG_INFO)
        flogr2(5,2) = .true.  !Charnock parm. (used to compute z0)
...
!
! -------------------------------------------------------------------- /
! Surface frictional velocity (sea-state dependent wind stress)
! XYC added
      i1 = FieldIndex( expFieldName, 'uustss', rc )
      if (ESMF_LogFoundError(rc, PASSTHRU)) return
      i2 = FieldIndex( expFieldName, 'vustss', rc )
      ...
      if ( expFieldActive(i1) .and. &
           expFieldActive(i2) ) then
!         write(*,*) "debug: Call into FricVel"
        call CalcFricVel( expField(i1),expField(i2), rc )
        if (ESMF_LogFoundError(rc, PASSTHRU)) return
      endif
!
! -------------------------------------------------------------------- /
...
!
      ...
      write(*,*) "debug: natGridIsLocal=", natGridIsLocal
!
...
!/
      ...
      integer :: isea, jsea
      real    :: emean, fmean, fmean1, wnmean, amax, ustar, ustdr, &
                 tauwx, tauwy, cd, z0, fmeanws, wnd_pa
      logical :: llws(nspec)
...
!
      ...
      elseif (GTYPE.eq.UNGTYPE) then
      if ( LPDLIB == .FALSE. ) then
        wrlnField = ESMF_FieldCreate( natGrid, natArraySpec1D, &
        staggerLoc=natStaggerLoc, rc=rc )
        if (ESMF_LogFoundError(rc, PASSTHRU)) return
      endif
      endif
      ...
      if ( LPDLIB == .FALSE. ) then
        ...
        if (ESMF_LogFoundError(rc, PASSTHRU)) return
         ...
         call ESMF_FieldGet( wrlField, farrayPtr=wrln, rc=rc )
         if (ESMF_LogFoundError(rc, PASSTHRU)) return
      endif
      ...
      if ( LPDLIB == .FALSE. ) then
        ...
        jsea_loop2: do jsea = 1,np
          isea = iplg(jsea)
          wrln(jsea) = charn(jsea)*USTFLD(jsea)**2/grav
          ! if sea-state dependnet wind stress iteration
          ! failed:(use bulk z0)
          if ( QCFLG(jsea) .ne. 1 ) then
             ! TO-DO: add the bulk Z0 function here.(ADCIRC/GFDL/other?)
             call wnd2Z0Mbulk(U10(isea)/WIND_FAC, wrln(jsea))
          endif
       enddo jsea_loop2
      endif
...
!     layout, whereas for unstructured meshes it is a 1D array
      ...
      if ( LPDLIB == .FALSE. ) then
        ...
        if (ESMF_LogFoundError(rc, PASSTHRU)) return
         ...
         call ESMF_FieldGet( ustxField, farrayPtr=ustxn, rc=rc )
         if (ESMF_LogFoundError(rc, PASSTHRU)) return
         call ESMF_FieldGet( ustyField, farrayPtr=ustyn, rc=rc )
         if (ESMF_LogFoundError(rc, PASSTHRU)) return
      endif
...
!#ifdef USE_W3OUTG_FOR_EXPORT
!         for ssd wind stress:
          ustxn(jsea)= USTFLD(isea)*cos(USTDIRFLD(isea))
          ustyn(jsea)= USTFLD(isea)*sin(USTDIRFLD(isea))
!           ustxn(jsea)=1.0
!           ustyn(jsea)=1.0
          !use qcflag to further perform quality control of the
          !frictional velocity;
          if (QCFLG(isea).ne.INT(1)) then
           ! use bulk frictional velocity for ADCIRC
           ! TO-DO: remove WIND_FAC when I complete a test with
           ! betamax=1.33
             call wnd2ustadcbulk(U10(isea)/WIND_FAC, U10D(isea)/WIND_FAC,ustxn(jsea), ustyn(jsea))
          endif
!#else
...
!#endif
        ...
        jsea_loop2: do jsea = 1,np
          isea = iplg(jsea)
          ustxn(jsea)= USTFLD(jsea)*cos(USTDIRFLD(jsea))
          ustyn(jsea)= USTFLD(jsea)*sin(USTDIRFLD(jsea))
 
          !use qcflag to further perform quality control of the
          !frictional velocity;
          if ( (QCFLG(jsea) .ne. INT(1)) ) then
          ! use bulk frictional velocity for ADCIRC
             call wnd2ustadcbulk(U10(isea)/WIND_FAC, U10D(isea)/WIND_FAC,ustxn(jsea), ustyn(jsea))
          endif
        enddo jsea_loop2
...
#endif
!/
        END SUBROUTINE CalcFricVel
!
! --------- Extra subroutine added by XYC to compute bulk Z0 and ust
       subroutine wnd2ustadcbulk(u10, u10dir, ustx, usty)
         implicit none
         real,intent(in):: u10, u10dir
         real,intent(out):: ustx, usty
         real:: ustmag
         real:: Cdadc, capval
...
!
       subroutine wnd2Z0Mbulk(U10, Z0BLK)
          implicit none
          real, intent(in)::  U10
          real, intent(out)::  Z0BLK
          real:: Cdadc, capval
          real, parameter:: kappa=0.40
          real(kind=8):: a(9),b(9)
 
      a(1)=1.044183210405817* 10**(-12)
      a(2)=-5.707116220939218* 10**(-11)
      a(3)=8.005722172810571* 10**(-10)
      a(4)=6.322045801589353* 10**(-9)
      a(5)=-2.422002988137712* 10**(-7)
      a(6)=2.269200594753249* 10**(-6)
      a(7)=-6.029592778169796* 10**(-6)
      a(8)=8.882284703541603* 10**(-6)
      a(9)=-2.371341185499601* 10**(-6)
 
      b(1)=1.814407011197660* 10**(-15)
      b(2)=-1.602907562918788* 10**(-13)
      b(3)=-3.351205313520358* 10**(-11)
      b(4)= 6.036179295940524* 10**(-9)
      b(5)=-3.725481686822030* 10**(-7)
      b(6)= 1.059761705898929* 10**(-5)
      b(7)=-1.375241830530252* 10**(-4)
      b(8)= 8.538858261732818* 10**(-4)
      b(9)=-1.936638976963742* 10**(-3)
         ...
         ! ust^2 = Cd*U10^2
         Cdadc=(0.75+0.067*u10)*0.001
         ! compute Z0 (at 10m height) from Cd:
       !  Z0BLK = 10 / exp(kappa/sqrt(Cdadc))
         ! If the GFDL Cd needs to be used, the following Z0 can be
         ! used:
         if ( 1 .eq. 1 ) then
            if (U10 .le. 0.4) then
               Z0BLK=4*10**(-7)
            elseif ( U10 .le. 9.3 ) then
               Z0BLK = a(9) + a(8)*U10 + a(7)*U10**2 + a(6)*U10**3 + &
                       a(5)*U10**4 + a(4)*U10**5 + a(3)*U10**6 + &
                       a(2)*U10**7 + a(1)*U10**8
            elseif ( U10 .lt. 60 ) then
               Z0BLK = b(9) + b(8)*U10 + b(7)*U10**2 + b(6)*U10**3 + &
                       b(5)*U10**4 + b(4)*U10**5 + b(3)*U10**6 + &
                       b(2)*U10**7 + b(1)*U10**8
            else
              Z0BLK = 1.3025*10**(-3)
            endif
