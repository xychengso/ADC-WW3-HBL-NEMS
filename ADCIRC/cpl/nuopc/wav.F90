...
! XYC: add SSD wind stress pack and send from here 
    ...
    call State_getFldPtr(ST=exportState,fldname='taux',fldptr=dataPtr_ssdwsx,rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    call State_getFldPtr(ST=exportState,fldname='tauy',fldptr=dataPtr_ssdwsy,rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
