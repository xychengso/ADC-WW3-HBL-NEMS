...
! XYC: changed the order of this line;
! Note: with this order, WVNSX1 is taken from WVNSX2 from the previous coupling time step        
!        WVNSX1 = WVNSX2
!        WVNSY1 = WVNSY2
       ...
       ! (also at current time);
        ...
        where(abs(WVNSX2).le.10**(-5)) WVNSX2=0.0
        where(abs(WVNSY2).le.10**(-5)) WVNSY2=0.0
...
!!!!#endif
   ...
   do num = 1,fldsToAdc_num
    ...
    if ( meteo_forcing) then
        ...
        call State_getFldPtr_(ST=importState,fldname='izwh10m',fldptr=dataPtr_izwh10m,rc=rc,dump=.true.,timeStr=timeStr)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=__LINE__, &
          file=__FILE__)) &
          return  ! bail out
...
!        !The original code: WVNX1(used by the current time in ADCIRC is set by
!        WVNX2 from previous time step)
!        if (.ne. first_exchange) then
!           WVNX1 = WVNX2   
!           WVNY1 = WVNY2  
!           PRN1  = PRN2
!        endif
...
!         PRN2  = PRN1
        ...
        !TODO: unit check
        do i1 = 1, mdataOut%NumOwnedNd, 1
!            WVNX1(mdataOut%owned_to_present_nodes(i1)) =  dataPtr_izwh10m(i1) !* 0.0  !zonal is u-comp or x-comp
            WVNX2(mdataOut%owned_to_present_nodes(i1)) =  dataPtr_izwh10m(i1) !* 0.0  !zonal is u-comp or x-comp
        end do

        do i1 = 1, mdataOut%NumOwnedNd, 1
!            WVNY1(mdataOut%owned_to_present_nodes(i1)) =  dataPtr_imwh10m(i1) !* 0.0  !Meridionalis v-comp or y-comp
            WVNY2(mdataOut%owned_to_present_nodes(i1)) =  dataPtr_imwh10m(i1) !* 0.0  !Meridionalis v-comp or y-comp
        end do
        
        do i1 = 1, mdataOut%NumOwnedNd, 1
!            PRN1(mdataOut%owned_to_present_nodes(i1) ) = dataPtr_pmsl(i1) / (1025 * 9.81)    !convert Pascal to mH2O
...
!
         ...
         ! XYC added to resolve problem with a smaller WW3 mesh         
...
!        rc=rc)
      ...
      call State_dump(ST=exportState,fldname='zeta', rc=rc,timeStr=timeStr)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return  ! bail out
...
!      call State_dump(ST=exportState,fldname='velx', rc=rc,timeStr=timeStr)
!      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
!        line=__LINE__, &
!        file=__FILE__)) &
!        return  ! bail out
...
!      call State_dump(ST=exportState,fldname='vely', rc=rc,timeStr=timeStr)
!      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
!        line=__LINE__, &
!        file=__FILE__)) &
!        return  ! bail out
...
!    ! local variables
    type(ESMF_Field) :: lfield
    integer :: lrc
    character(len=*),parameter :: subname='(ADC: State_dump)'
!
!    rc=EMSF_SUCCESS
!
    call ESMF_StateGet(ST, itemName=trim(fldname), field=lfield, rc=lrc)
...
!
    ...
    call ESMF_FieldWrite(lfield, fileName='field_ocn_'//trim(fldname)//trim(timeStr)//'.nc', &
        rc=rc,overwrite=.true.)
     if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, &
         file=__FILE__)) &
