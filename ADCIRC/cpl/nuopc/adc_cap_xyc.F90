...
module adc_cap
  ...
  USE GLOBAL,  ONLY: WSX1, WSY1
  USE GLOBAL,  ONLY: WSX2, WSY2
  ! XYC finished adding.
...
!    call fld_list_add(num=fldsToAdc_num, fldlist=fldsToAdc,stdname="sea_state_dependent_wind_stress", shortname="ssdwsx")
!    call fld_list_add(num=fldsToAdc_num, fldlist=fldsToAdc,stdname="sea_state_dependent_wind_stress", shortname="ssdwsy")
  ...
  subroutine ModelAdvance(model, rc)
   ...
   do num = 1,fldsToAdc_num
    ...
    if (wave_forcing) then
        ...
        !   components)
!        IF(.NOT.ALLOCATED(ADC_SSDWSX)) ALLOCATE(ADC_SSDWSX(1:NP,1:2))
!        IF(.NOT.ALLOCATED(ADC_SSDWSY)) ALLOCATE(ADC_SSDWSY(1:NP,1:2))
        
        IF(.NOT.ALLOCATED(WSX1)) ALLOCATE(WSX1(1:NP))
        IF(.NOT.ALLOCATED(WSY1)) ALLOCATE(WSY1(1:NP))
        
        IF(.NOT.ALLOCATED(WSX2)) ALLOCATE(WSX2(1:NP))
        IF(.NOT.ALLOCATED(WSY2)) ALLOCATE(WSY2(1:NP))
        ...
        do i1 = 1, mdataOut%NumOwnedNd, 1
            WSX2(mdataOut%owned_to_present_nodes(i1),1) = dataPtr_ssdwsx(i1)
        end do

        do i1 = 1, mdataOut%NumOwnedNd, 1
            WSY2(mdataOut%owned_to_present_nodes(i1),1) = dataPtr_ssdwsy(i1)
        end do
        
        ! Ghost nodes update
        call UPDATER( WSX1(:), WSY1(:), 2)
        call UPDATER( WSX2(:), WSY2(:), 2)
...
!!!!#ifdef NO_COMPILE00000
   ...
   !     if ((currTime > BeforeCaribbeanTime) .and. (currTime < AfterCaribbeanTime)) then
            ...
            write(info,*) subname, 'in cap after maxval(WSX2)', maxval(WSX2)
            call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, rc=dbrc)

            write(info,*) subname, 'in cap after maxval(WSY2)', maxval(WSY2)
            call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, rc=dbrc)
